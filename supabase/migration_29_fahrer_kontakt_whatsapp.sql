-- ============================================================================
-- Migration 29: Fahrername und -telefonnummer bei den Fahrten-Suchergebnissen,
-- der Fahrt-Detailansicht und "Meine Buchungen" mit ausliefern, damit
-- Mitfahrer den Fahrer direkt per WhatsApp kontaktieren können.
-- ============================================================================
...
-- ----------------------------------------------------------------------------
-- fn_get_trip_details: Fahrername/-telefonnummer mit ausliefern
-- ----------------------------------------------------------------------------
...
begin
  ...
  select tr.route_id, tr.total_seats, c.name, c.notes, tr.closed, tr.driver_id, d.name, d.phone, d.payment_info
    into v_route_id, v_total, v_car_name, v_car_notes, v_closed, v_driver_id, v_driver_name, v_driver_phone, v_driver_payment
  from trips tr
  left join cars c on c.id = tr.car_id
  left join drivers d on d.id = tr.driver_id
...
-- ----------------------------------------------------------------------------
-- fn_list_my_bookings: Fahrername/-telefonnummer mit ausliefern
-- ----------------------------------------------------------------------------
