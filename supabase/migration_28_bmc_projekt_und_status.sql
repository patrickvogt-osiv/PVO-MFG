-- ============================================================================
-- Migration 28: Buy Me a Coffee — Umbau von "ein Link pro Person" auf
-- "ein Projekt-Link (Admin) + Abo-Status pro Fahrer/Mitfahrer".
--
-- - Neue Tabelle app_settings: generische Admin-Einstellungen (Key/Value),
--   aktuell genutzt für den einen Buy-Me-a-Coffee-Projekt-Link.
-- - drivers/people: buymeacoffee_link entfällt, stattdessen
--   bmc_subscription_active (boolean) und bmc_last_payment_date (Datum).
--   Diese werden vorerst manuell vom Admin gepflegt; später kann hier die
--   Buy-Me-a-Coffee-Webhook-Integration automatisch reinschreiben.
-- ============================================================================
-- Im Supabase SQL-Editor ausführen.
-- ============================================================================

-- ----------------------------------------------------------------------------
-- Generische Admin-Einstellungen (Key/Value)
-- ----------------------------------------------------------------------------
...
-- Projekt-Link), damit auch Fahrer/Mitfahrer (anon-Key) ihn sehen können.
...
-- ----------------------------------------------------------------------------
-- drivers/people: Einzel-Link entfernen, Abo-Status ergänzen
-- ----------------------------------------------------------------------------
alter table drivers drop column if exists buymeacoffee_link;
alter table drivers add column if not exists bmc_subscription_active boolean not null default false;
alter table drivers add column if not exists bmc_last_payment_date   date;

alter table people drop column if exists buymeacoffee_link;
alter table people add column if not exists bmc_subscription_active boolean not null default false;
alter table people add column if not exists bmc_last_payment_date   date;

-- ----------------------------------------------------------------------------
-- fn_driver_update_profile: Buy-Me-a-Coffee-Parameter entfernen (nicht mehr
-- selbst editierbar durch den Fahrer)
-- ----------------------------------------------------------------------------
drop function if exists fn_driver_update_profile(text, text, text, numeric, text, text, text);
...
-- ----------------------------------------------------------------------------
-- fn_driver_list_trips: BMC-Abo-Status (eigener + Projekt-Link) mit ausliefern
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- fn_person_update_profile: Buy-Me-a-Coffee-Parameter entfernen (nicht mehr
-- selbst editierbar durch den Mitfahrer)
-- ----------------------------------------------------------------------------
drop function if exists fn_person_update_profile(text, text, text, text);
...
-- ----------------------------------------------------------------------------
-- fn_list_open_trips: BMC-Abo-Status (eigener + Projekt-Link) mit ausliefern
-- ----------------------------------------------------------------------------
